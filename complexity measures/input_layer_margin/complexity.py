...
def complexity(model, ds):
  ...
  for i, (x,y) in enumerate(ds.batch(32)):
    output = model(x)
    num_classes = output.shape[1]
    if i==0:
      break
  temp_vec = np.zeros((num_classes, num_classes - 1)).astype(int)
  ...
  for i, (x, y) in enumerate(ds.batch(16)):
    ...
    with tf.GradientTape() as tape:
      output = model(x, tape=tape)
    #output = model(x)
    jacob = tape.batch_jacobian(output, x).numpy()
    output = output.numpy()

    act_class = output[np.arange(0, 16).reshape(-1,1), y.numpy().reshape(-1,1)]
    other_class = output[np.arange(0, 16).reshape(-1,1), temp_vec[y.numpy()]]
    temp = act_class - other_class

    jacob_act_class = jacob[np.arange(0, 16).reshape(-1,1), y.numpy().reshape(-1,1)]
    jacob_other_class = jacob[np.arange(0, 16).reshape(-1,1), temp_vec[y.numpy()]]
    temp2 = jacob_act_class - jacob_other_class
    temp2 = temp2.reshape(temp2.shape[0], temp2.shape[1], -1)
    temp2 = np.linalg.norm(temp2, axis=2)

    fin = temp/temp2
    if margin is None:
      margin = np.min(fin, axis=1)
    else:
      margin = np.concatenate([margin, np.min(fin, axis=1)])
